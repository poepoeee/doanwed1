...
function addToCart(product) {
  ...
  let productIndex = listProduct.findIndex(
    (element) => element.id === productId
  );
  console.log(productIndex);
  if (
    (duplicateIndex = findByProductId(productId, listCart[currentUserId])) != -1
  ) {
...
function renderCart() {
      ...
      HTML += `
        <div class="product">
              <ul class="title-cart">
                ...
                <li class="total">£${
                  x.price.split("£")[1] * x.soluong
                }<i onclick="deleteProductFromCart(this)" class="ti-close hover_icon"></i></li>
  ...
  subTotalObject.firstChild.innerText = "£" + totalCost;
...
function changeCount(object) {
  let currentId = JSON.parse(localStorage.getItem("user")).id;
  let listCart = JSON.parse(localStorage.getItem("carts"));
  let productCartIndex = listCart[currentId].findIndex(
    (x) => x.id == object.getAttribute("id")
  );
  ...
  localStorage.setItem("carts", JSON.stringify(listCart));
  object.parentElement.nextElementSibling.innerHTML =
    "£" + numberPrice * object.value + `<i class="ti-close hover_icon"></i>`;
}
function deleteProductFromCart(object) {
  let deleteId =
    object.parentElement.previousElementSibling.firstElementChild.getAttribute(
      "id"
    );
  let currentId = JSON.parse(localStorage.getItem("user")).id;
  let listCart = JSON.parse(localStorage.getItem("carts"));
